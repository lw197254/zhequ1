...
#import "UITableView+DefaultSet.h"

@implementation UITableView (DefaultSet)
+(void)load{
    //只执行一次这个方法
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL originalSelector2 = @selector(initWithFrame:style:);
        SEL originalSelector3 = @selector(awakeFromNib);
        SEL swizzledSelector = @selector(YHBaseInit);
        SEL swizzledSelector2 = @selector(YHBaseInitWithFrame:style:);
...
- (instancetype)YHBaseInit
{
    ...
    [self setSeparatorColor:BlackColorE3E3E3];
...
-(instancetype)YHBaseInitWithFrame:(CGRect)rect style:(UITableViewStyle)style{
    id __self = [self YHBaseInitWithFrame:rect style:style];
    [self setSeparatorColor:BlackColorE3E3E3];
    //    UIFont * font = [UIFont fontWithName:@"PingFangSC-Regular" size:self.font.pointSize];
    ////    if (self.font.pointSize ==15) {
    ////        font = [UIFont fontWithName:@"PingFangSC-Regular" size:17];
