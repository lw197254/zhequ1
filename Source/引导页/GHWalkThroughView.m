...
- (void) setup
{
    ...
    //添加约束
    [_bgFrontLayer mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(bgView);
        make.top.equalTo(bgView.mas_top).offset(80);
    }];
    
    [_bgBackLayer mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(bgView);
        make.top.equalTo(bgView.mas_top).offset(80);
    }];
...
- (void) orientFooter
{
    if (self.walkThroughDirection == GHWalkThroughViewDirectionVertical) {
        ...
        if (!isRotated) {
            ...
            frame.origin.y = butonFrame.origin.y+butonFrame.size.height - frame.size.height-40;
        ...
        if (isRotated) {
            ...
            self.pageControl.transform = CGAffineTransformRotate(self.pageControl.transform, -M_PI_2);
            self.pageControl.frame = CGRectMake(0, self.bounds.size.height - 80 - SafeAreaBottom, self.bounds.size.width, 20);
...
- (void)buildFooterView {
    ...
    self.pageControl.currentPageIndicatorTintColor = BlueColor447FF5;
...
{
    if ([self.dataSource numberOfPages]) {
        return 1;
    }
    return 0;
}
...
//    int page = floor((contentOffset - pageMetric / 2) / pageMetric) ;
     int page = (contentOffset+kwidth/2)/kwidth;
    ...
    if (page == 2) {
     ...
     NSLog(@"GHWalkThroughPageCellpage:%ld",page);
}
...
- (void) showInView:(UIView *)view animateDuration:(CGFloat) duration
{
    ...
    self.pageControl.numberOfPages = [self.dataSource numberOfPages]-1;;
