...
// update
#define ITUNES_CHECKVERSION_URL    @"http://itunes.apple.com/lookup?id=%@&country=cn"
...
#define newUpdateOtherButtonTitles NSLocalizedStringWithDefaultValue(@"umUpdateOK",nil,[NSBundle mainBundle], @"前往更新",@"Umeng Update");
...
//        [self performSelectorOnMainThread:@selector(appUpdateWithAlert:) withObject:self.updateInfo waitUntilDone:YES];
        self.isNeedUpdate  = NeedUpdate;
    }else{
        self.isNeedUpdate = NoUpdate;
    }
}
...
- (void)appUpdateWithAlert:(NSDictionary *)objectData {
    ...
    [alert showWithTitle:[self.updateTitle stringByAppendingString:newVersion] message:message cancelButtonTitle:self.updateCancelButtonTitle confirmButtonTitle:self.updateOtherButtonTitles cancel:^{
