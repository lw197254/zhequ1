...
-(UIButton *)initNavigationButton:(UIImage *)image with:(float) with{
    ...
    if ( IOS7_OR_LATER )
    {
        buttonFrame = CGRectMake(0, 0, with, NAV_BAR_HEIGHT);
    }
    else
    {
        buttonFrame = CGRectMake(0, 0, with, NAV_BAR_HEIGHT);
...
-(UIButton *)initNavigationButtonWithTitle:(NSString *)str color:(UIColor *)color height:(float) height {
    ...
    CGSize titleSize = [str boundingRectWithSize:CGSizeMake(999999.0f, NAV_BAR_HEIGHT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:11]} context:nil].size;
    
    if ( IOS7_OR_LATER )
    {
        buttonFrame = CGRectMake(0, 0, titleSize.width+1, height);
    }
    else
    {
        buttonFrame = CGRectMake(0, 0, titleSize.width+10.0f, height);
    }
